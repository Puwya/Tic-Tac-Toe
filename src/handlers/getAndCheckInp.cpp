...
void getAndCheckInp(
  ...
  do {
    if (token == 'X') {
      ...
      updatedBoard =
          checkAiInput(difficulty, boardAr, token);  // returns true if Ai input was valid
    }

    if (onePlayerDeterminer != 'd' ||
        token == 'X')  // if one player mode is active than only on token X will this
                       // control flow statement run
    {
      if (row < 3 && row > -1 && col < 3 && col > -1) {
        if (isspace(boardAr[row][col])) {  // if current element in the 2d array is taken
                                           // by a space then its available
      ...
      !updatedBoard);  // once board is updated to true this expression will be not true
}