...
bool normalAiBlocking(char boardAr[][3], const char token) {
  if (boardAr[0][0] == 'X' && boardAr[0][1] == 'X' && boardAr[0][2] != 'O') {
    // if boardAr at position [0][0] is equal to X and so is [0][1] then we check if Ai
    // has already blocked [0][2] and if not than Ai blocks same logic applies to the rest
    // of the ladder
    thinking(boardAr);
    boardAr[0][2] = token;
    return true;
